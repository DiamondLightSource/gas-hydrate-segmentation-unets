"""Script to train a 3D-unet for segmentation when given data and ground
...
def prepare_batch(batch, device):
    """Prepares data batches for passing into the U-net.

    Args:
        batch : Batch of data from dataloader.
        device (int): Device to send the data to.

    Returns:
        tuple: The input data and the corresponding target data.
    """
...
def output_loss_fig_data(train_loss, valid_loss, avg_eval_scores, data_path):
    """Saves out a plot with training and validation losses vs epoch. Also outputs
    losses and evalutation scores to a CSV file. 

    Args:
        train_loss (list): Training losses.
        valid_loss (list): Validation losses.
        avg_eval_scores (list): Evalution metric scores.
        data_path (pathlib.Path): Path to the data output directory.
    """
...
def plot_validation_slices(predicted_vol, data_path, filename):
    """Saves out a figure of central slices to compare ground truth and model prediction.

    Args:
        predicted_vol (numpy.array): The predicted label volume.
        data_path (pathlib.Path): Path to the ground truth label volume, HDF5 format.
        filename (str): Filename to use for prepending to output image name.
    """
    columns = 3
    rows = 2
...
if not multilabel:
    ...
    MODEL_DICT['final_sigmoid'] = True
else:
    MODEL_DICT['out_channels'] = NUM_CLASSES
    MODEL_DICT['final_sigmoid'] = False
